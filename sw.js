...
def _apply_adaptive_context(strategy, user, pair, timeframe, tf_df):
    ...
    # Exact-match preference: BALANCED near-matches remain WATCH unless this exact
    # strategy+pair+timeframe combination has a meaningful STRONG resolved record.
    if strategy.get("balanced_acceptance_applied") and not (profile.get("sample_size",0) >= ADAPTIVE_CONTEXT_MIN_SAMPLE and profile.get("smoothed_win_rate",0) >= 64.0):
        strategy.update({"raw_signal_before_near_match_gate":strategy.get("signal"),"signal":"NO SIGNAL","pattern_direction":"NONE",
                         "next_candle_color":"NONE","no_trade":True,"near_match_watch_only":True,
                         "reason":f"WATCH ONLY · Near-match Type {ptype} requires a strong resolved record for {pair} {timeframe}; current status {profile.get('status')} (n={profile.get('sample_size')})."})
        return strategy
...
SK25_ENGINE_VERSION = "RAJA_AI_V47_SK25_BALANCED_CONFIRM"
...
def normalize_scan_options(_raw):
    ...
    return {
        ...
        "vol_min": 0.0,
        "vol_max": 999.0,
        "good_near_match": 80.0,
        "caution_near_match": 88.0,
...
def _apply_balanced_acceptance(strategy, tf_df, timeframe, market_name, opts):
    ...
    threshold = float(opts.get("good_near_match", 80.0) if health["grade"] == "GOOD" else opts.get("caution_near_match", 88.0))
...
V46_MIN_CONFIDENCE = max(60.0, min(95.0, float(os.environ.get("RAJA_V46_MIN_CONFIDENCE", os.environ.get("RAJA_V47_MIN_CONFIDENCE", "75")))))
...
def _v46_indicator_snapshot(df):
    """Return closed-candle indicator state. Pure pandas; no TA dependency."""
    if df is None or getattr(df, "empty", True) or len(df) < max(28, V46_EMA_SLOW + 5):
        return {"ready": False, "reason": "Not enough closed candles for V46 indicator warm-up."}
    try:
        close = pd.to_numeric(df["Close"], errors="coerce").astype(float)
        high = pd.to_numeric(df["High"], errors="coerce").astype(float)
        low = pd.to_numeric(df["Low"], errors="coerce").astype(float)
        if close.isna().sum() or high.isna().sum() or low.isna().sum():
            clean = pd.DataFrame({"Close":close,"High":high,"Low":low}).dropna()
            close, high, low = clean["Close"], clean["High"], clean["Low"]
        if len(close) < max(28, V46_EMA_SLOW + 5):
            return {"ready": False, "reason": "Not enough clean candles for V46 indicator warm-up."}
        ...
        atr_pct = (atr / close.abs().replace(0.0, 1e-12)) * 100.0
        ...
        vals = {
            ...
            "ema_fast": float(ema_fast.iloc[last]),
            "ema_slow": float(ema_slow.iloc[last]),
            ...
            "atr_pct_median": float(atr_ref.iloc[last]) if atr_ref.iloc[last] == atr_ref.iloc[last] else float(atr_pct.iloc[last]),
        }
        if not all(v == v for k,v in vals.items() if isinstance(v, float)):
            return {"ready": False, "reason": "Indicator calculation returned incomplete values."}
        return vals
    except Exception as exc:
        return {"ready": False, "reason": f"Indicator calculation failed: {type(exc).__name__}."}
...
def _v46_direction_checks(snapshot, signal):
    ...
    rsi_ok = (52.0 <= rsi <= 72.0) if up else (28.0 <= rsi <= 48.0)
    ...
    volatility_ok = 0.55 <= atr_ratio <= 1.85
...
def _v46_apply_technical_confirmation(strategy, base_df, tf_df, timeframe):
    ...
    # V47: only the selected timeframe warm-up is mandatory. HTF warm-up is
    # optional until enough history arrives, preventing the old 40% lockout.
    warmup_failed=not bool(snap.get("ready"))
    if warmup_failed or hard_opposition or score < V46_MIN_CONFIDENCE:
        original=signal
        reasons=[]
        if warmup_failed: reasons.append("indicator warm-up incomplete")
        ...
        strategy.update({
            ...
            "reason":"NO TRADE · V47 balanced confirmation gate: " + "; ".join(reasons) + ".",
        })
    else:
        strategy["v46_technical_blocked"]=False
        strategy["reason"]=(str(strategy.get("reason") or "").rstrip(".") +
                            f" · V47 confirmed {score:.0f}% (EMA/RSI/MACD/ATR + HTF filter).")
    return strategy
...
def calculate_live_strategy_signal(pair, selected_expiry=None, scan_options=None, bridge_user=None, broker=None, chart_count=32):
    ...
    tf_df=build_timeframe(base_df,TIMEFRAMES[tf])
    # V50: the SK25 pattern engine scans the exact same latest CLOSED candles
    # that are returned to the live chart. Technical indicators may still use
    # the full closed-candle history for warm-up (EMA/MACD/ATR need more rows),
    # but the actual candle-pattern decision is made from the visible chart set.
    ...
    result.update({
        ...
        "confirmation_mode":"V47 · SK25 + EMA/RSI/MACD/ATR + BALANCED HIGHER-TIMEFRAME FILTER","scan_mode":opts.get("mode","SK25_BALANCED"),"scan_thresholds":opts,
        "data_age":round(float(data_age),2) if data_age is not None else None,
        ...
        "chart_preview":serialize_candles(chart_scan_df,visible_count),"engine":SK25_ENGINE_VERSION,"pattern_library":"RAJA SK25 Pattern Type 1-25","pattern_library_size":SK25_PATTERN_LIBRARY_SIZE,
        "visible_candles_analyzed":True,"pattern_scan_candles":int(len(chart_scan_df)),"requested_chart_candles":visible_count,
        "technical_warmup_candles":int(len(tf_df)),"chart_signal_sync":"SAME CLOSED OHLC FOR SK25 PATTERN SCAN",
        ...
        "deep_quality_score":float(strategy.get("pattern_priority") or 0),"calibration_status":strategy.get("calibration_status") or "V47 BALANCED TECHNICAL FILTER", "calibrated_confidence":float(strategy.get("calibrated_confidence") or strategy.get("model_confidence") or 0.0),
...
# FINAL V64 market-source override:
# normal Forex is Finnhub/OANDA live; existing OTC/crypto behavior stays intact.
_legacy_get_market_data_v60 = get_market_data
def get_market_data(pair, bridge_user=None, broker=None):
    clean = str(pair or "").strip()
    if clean in FINNHUB_FOREX_SYMBOLS and "(OTC)" not in clean:
        return _finnhub_live_base_df(clean, count=1800)
    return _legacy_get_market_data_v60(clean, bridge_user=bridge_user, broker=broker)

def _append_live_forming_candle_to_result(result, pair, tf, chart_count):
    """Chart shows the forming Finnhub candle; RAJA strategy still scans only closed bars."""
    if not isinstance(result, dict) or pair not in FINNHUB_FOREX_SYMBOLS:
        return result
    snap = _finnhub_live_snapshot(pair)
    forming = (snap.get("forming") or {}).get(str(_finnhub_tf_minutes(tf)))
    if not isinstance(forming, dict):
        return result
    preview = list(result.get("chart_preview") or [])
    item = {
        "t": int(forming["t"]),
        "o": float(forming["o"]),
        "h": float(forming["h"]),
    ...
    if preview and int(preview[-1].get("t") or -1) == item["t"]:
        preview[-1] = item
    else:
        preview.append(item)
    result["chart_preview"] = preview[-max(25, min(300, int(chart_count or 50))):]
    result["source"] = "Finnhub/OANDA LIVE + Yahoo closed candles"
    result["source_mode"] = "finnhub_oanda_live"
    result["provider_symbol"] = FINNHUB_FOREX_SYMBOLS[pair]
    result["data_age"] = snap.get("age_seconds")
    result["finnhub_live_price"] = snap.get("price")
    result["finnhub_tick_age"] = snap.get("age_seconds")
    result["forming_candle_excluded"] = True
    result["chart_last_candle_live"] = True
    result["chart_signal_sync"] = "CLOSED CANDLES=YAHOO · LIVE/FORMING CANDLE=FINNHUB/OANDA"
    return result
...
def live_chart_data():
    """Final V61 live chart: normal Forex is Finnhub/OANDA only; other markets keep legacy feeds."""
    data = request.get_json(silent=True) or {}
    auth, error = _auth_session(data)
    if error:
        return error
    ...
    try:
        ...
        if selected_pair in FINNHUB_FOREX_SYMBOLS and "(OTC)" not in selected_pair:
            result = _append_live_forming_candle_to_result(
                result, selected_pair, selected_expiry, chart_count
            )
        return jsonify({
            "status":"success",
            "data":result,
            "read_only":True,
            "note":"V61 strict mode: normal Forex uses Finnhub/OANDA only. Signals use closed candles; the displayed last candle may still be forming.",
        })
    except Exception as exc:
